  ...
  return (
    <ProtectedRoute requiredRole="VENDOR">
      <div className="min-h-screen w-full bg-gradient-to-br from-purple-100 via-orange-50 to-white flex flex-col items-center justify-center px-4 py-8 relative overflow-hidden">
        ...
        <h2 className="text-3xl font-extrabold mb-8 bg-gradient-to-r from-purple-600 to-orange-500 bg-clip-text text-transparent drop-shadow-lg tracking-tight">Vendor Profile</h2>
        {loading ? (
          <div className="flex items-center justify-center min-h-[200px]">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-purple-600 mr-3"></div>
            <span className="text-purple-700 text-lg">Loading profile...</span>
          </div>
        ) : error ? (
          <p className="text-red-500 text-lg font-semibold">{error}</p>
        ) : profile ? (
          <div className="w-full max-w-2xl bg-white/70 backdrop-blur-lg rounded-3xl shadow-2xl p-0 overflow-hidden animate-fade-in-up border border-purple-100">
            {/* Card Header with Avatar */}
            <div className="flex flex-col items-center justify-center bg-gradient-to-r from-purple-600 to-orange-500 p-10 pb-8 relative">
              <div className="w-32 h-32 rounded-full bg-white/80 flex items-center justify-center shadow-2xl mb-4 ring-8 ring-purple-200/40">
                <span className="text-6xl font-extrabold text-purple-600 drop-shadow-lg">{profile.name?.charAt(0).toUpperCase()}</span>
              </div>
              <div className="flex flex-col items-center gap-2 mt-2">
                <div className="text-3xl font-bold text-white tracking-wide flex items-center gap-3">
                  {profile.name}
                  <span className="ml-2 px-3 py-1 rounded-full text-sm font-bold bg-orange-200 text-orange-800 shadow">Active</span>
                </div>
                {profile.id && <div className="text-xs text-orange-100 mt-1">ID: {profile.id}</div>}
              </div>
            </div>
            ...
            <div className="p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  <svg className="w-6 h-6 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5.121 17.804A9 9 0 1112 21a9 9 0 01-6.879-3.196z" /></svg>
                  <span className="font-bold text-lg text-purple-700">{profile.name}</span>
                </div>
                <div className="flex items-center gap-4">
                  <svg className="w-6 h-6 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 12H8m8 0a4 4 0 11-8 0 4 4 0 018 0zm0 0v1a3 3 0 01-3 3H9a3 3 0 01-3-3v-1" /></svg>
              ...
              <div className="space-y-6">
                {profile.company && (
                  <div className="flex items-center gap-4">
                    ...
                    <span className="font-bold text-orange-700">{profile.company}</span>
                  </div>
                )}
                {profile.address && (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-gray-700">Role: <span className="font-semibold text-purple-600">Vendor</span></span>
                </div>
                <div className="flex items-center gap-4">
